...
class User{
    ...
    public function userRegistration($name, $username, $password, $email){
        ...
        if ($name == "" || $username =="" || $password =="" || $email == "") {
            echo "<span class='error'><br>Fields must not be empty...!</span>";  
            exit();

          }elseif(filter_var($email, FILTER_VALIDATE_EMAIL) === false){
            echo "<span class='error'><br>Invalid email address...</span>";  
            exit();
          }else{
            ...
            if ($chkresult != false ) {
                echo "<span class='error'><br>Email address already exists...!</span>";  
                exit();
            }else{
            $password = mysqli_real_escape_string($this->db->link, md5($password));
                ...
                if ($insert_row) {
                    echo "<span class='success'><br>Registration successfully...</span>";  
                    exit();

                }else{
                    echo "<span class='error'><br>Error, Registration not successfully...!</span>";  
                    exit();
                }
            }
          }
    ...
    public function userLogin($email, $password){
        ...
        if ($email == "" || $password =="") {
            ...
            $password = mysqli_real_escape_string($this->db->link, md5($password));
            $query  = "SELECT * FROM tbl_user WHERE email = '$email' AND 
            password = '$password'";
